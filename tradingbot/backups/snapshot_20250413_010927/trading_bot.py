...
# Initialize the news integrator with your platform adapter
...
# Initialize the news integrator but don't set platform_adapter yet
news_integrator = NewsIntegrator()


# Later in the code, after platform_adapter is initialized (likely in the trading_loop function)
def trading_loop():
    ...
    if threading.current_thread() is threading.main_thread():
        ...
        logger.info("Signal handlers not installed - not in main thread")
    ...
    # NOW share the platform adapter with the news integrator
    news_integrator.platform_adapter = platform_adapter
...
def can_sell_stock(symbol, config):
    ...
    min_hold_period = config.get("trading_bot", {}).get("min_hold_period", 86400)
